...
**Baseline:** uniform random sampling :contentReference[oaicite:1]{index=1}
...
The first term captures total predictive uncertainty, while the second estimates expected aleatoric uncertainty; their difference isolates **epistemic uncertainty**, which is the quantity active learning seeks to reduce. :contentReference[oaicite:2]{index=2}

### Computational Cost

BALD incurs substantially higher computational cost than single-pass uncertainty heuristics because each acquisition step requires **\( T = 20 \)** stochastic forward passes per unlabeled example, making it roughly **20× more expensive** than entropy- or margin-based methods. :contentReference[oaicite:3]{index=3}
...
Under these conditions, disagreement across stochastic forward passes does **not yet reliably reflect epistemic uncertainty**, causing BALD to query suboptimal samples and lag behind simpler heuristics. :contentReference[oaicite:4]{index=4}
...
Once uncertainty estimates become reliable, BALD consistently selects highly informative samples and achieves the **highest final test accuracy** across labeling budgets. :contentReference[oaicite:5]{index=5}

This behavior highlights a central trade-off:  
**BALD sacrifices early stability in exchange for superior long-term performance.**
...
- Simpler uncertainty methods are computationally efficient, while BALD and CSAL incur higher overhead :contentReference[oaicite:6]{index=6}

The underperformance of Core-Set is attributed to the limited geometric quality of MLP feature embeddings, which can cause the k-center algorithm to prioritize outliers rather than informative samples. :contentReference[oaicite:7]{index=7}
